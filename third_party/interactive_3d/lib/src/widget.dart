...
class Interactive3dState extends State<Interactive3d> {
  ...
  Widget _buildTextureWidget() {
    final selectionEnabled = widget.onSelectionChanged != null ||
        widget.preselectedEntities?.isNotEmpty == true ||
        widget.selectionSequence?.isNotEmpty == true ||
        widget.enableCache;
    return GestureDetector(
      // A showroom needs orbit/zoom but not entity selection. Avoid creating
      // highlight materials when the caller did not request selection.
      onTapUp: selectionEnabled ? _handleTapUp : null,
